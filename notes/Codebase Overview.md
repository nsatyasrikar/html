...
- Every element file (`div.go`, `p.go`, ...) follows one template: a `<Name>Props` struct plus a constructor calling `globalAttrs` + `elementNode{...}` — all hand-written, see [[Decisions Log#htmlgen removal]]
